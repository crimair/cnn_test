...
foreach { inst } $instances {
	...
	if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
		set local_pll_avl_phy_clk [ fpga_mem_mem_if_ddr3_emif_0_p0_get_or_add_clock_vseries \
	...
	if {[string compare -nocase $pll_driver_core_clock "_UNDEFINED_PIN_"] != 0} {
		set local_pll_driver_core_clk [ fpga_mem_mem_if_ddr3_emif_0_p0_get_or_add_clock_vseries \
		...
		foreach { ac_pin } $ac_pins {
			...
			if {[get_collection_size $ac_port] > 0} {
				if [ get_port_info -is_output_port $ac_port ] {
					...
					set_output_delay -min [fpga_mem_mem_if_ddr3_emif_0_p0_round_3dp [expr {$ac_min_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] $ac_port -add_delay

					# Specifies the maximum delay difference between the DQS pin and the address/control pins:
					set_output_delay -max [fpga_mem_mem_if_ddr3_emif_0_p0_round_3dp [expr {$ac_min_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] $ac_port -add_delay
				}
			}
		}
	}
	...
	if {$::GLOBAL_fpga_mem_mem_if_ddr3_emif_0_p0_pll_phase(PLL_NIOS_CLK) > 0} {
		if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
